...
# docker buildx build --tag starportsample --file .pi/Dockerfile --platform linux/arm64 --cache-from starportsample:cache --cache-to starportsample:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar starportsample
...
sudo bash -c "echo starportsample > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/starportsample.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/starportsample.img
